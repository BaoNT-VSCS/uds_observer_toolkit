...
class RunLogger:
    ...
    def result(self, *, testcase: str, target: str, step: str, request: bytes, response: bytes = b"", status: str = "", nrc: int | None = None, note: str = "") -> None:
        self.event(
            "uds_result",
            testcase=testcase,
            target=target,
    ...
    def close(self) -> None:
        if not self._csv_rows:
            return
        fields = ["testcase", "target", "step", "request", "response", "status", "nrc", "note"]
