    ...
    def request(
        ...
        *,
        ...
        try:
            ...
            response_text = f"<redacted len={len(response)}>" if redact_response and response else spaced(response)
            self.log.process(f"  {step:<22} RX  {response_text}  {result.status} {result.note}")
            return result
